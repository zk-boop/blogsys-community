                ...
                <div class="meta-left">
                  <div class="author-info">
                    <el-avatar
                      ...
                      :alt="`${article.authorNickname || article.authorUsername || '作者'}头像`"
                    ></el-avatar>
...
export default {
  ...
  setup(props) {
    ...
    const fetchArticles = async () => {
      ...
      error.value = null
      try {
        const response = await articleApi.getArticlesByCategory(props.id, currentPage.value - 1, pageSize.value)
    ...
    const fetchCategory = async () => {
      try {
        const response = await categoryApi.getCategoryById(props.id)
        if (response.data.success) {
          category.value = response.data.data
          document.title = `${category.value.name} - 分类文章 - 博客系统`
        }
      } catch (err) {
    ...
    const goToArticleDetail = (articleId) => {
      // 确保使用router.push而不是route.push (route是当前路由信息，router才是导航对象)
